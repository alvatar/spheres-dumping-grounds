...
;
; Message digest computation module
;
...
(export ; Generally used exports. (Typechecked.)
        ...
        ; These perform the underlying hash based on the string content in UTF-8 byte coding.
        ; ** Note that we currently do not do Unicode normalization.
        digest-string
        digest-substring

        digest-file ; For character decoding, uses Gambit's default char encoding for files setting.

        ; Conveniency wrappers.
        ; Autodetects if string or u8vector input. Support digesting both whole and partial input value.
        crc32
        md5
        sha-1
        sha-224
        ...
        ; Exports for performing digest process manually. (Not typechecked.)
...
(declare (block)
         ...
         (standard-bindings) ; (This one was introduced by Marc.)

         (extended-bindings)

         (not inline)        ; The logics code below is implemented in a way that already is
                             ; maximally inlined, so no need put load on the compiler to
                             ; check if it could inline additionally.

         (fixnum)            ; All operations in this module regard fixnums.
...
;;;----------------------------------------------------------------------------
; Moved most typically used exports into area where typechecking is maintained
; for their operation.
...
(define (make-conveniency-hash-wrapper algorithm)
  (lambda (value #!optional start end)
    (if (string? value)
...
(define crc32   (make-conveniency-hash-wrapper 'crc32  ))
(define md5     (make-conveniency-hash-wrapper 'md5    ))
(define sha-1   (make-conveniency-hash-wrapper 'sha-1  ))
(define sha-224 (make-conveniency-hash-wrapper 'sha-224))
(define sha-256 (make-conveniency-hash-wrapper 'sha-256))
...
(define-macro (vector-ref . args) `(##vector-ref ,@args)) ; Added 2011-10-30
(define-macro (vector-set! . args) `(##vector-set! ,@args)) ; Added 2011-10-30
